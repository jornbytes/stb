...
export default function ContactSubmissions() {
  ...
  const [emailModal, setEmailModal] = useState<Message | null>(null);
  ...
  async function markBehandeld(id: string) {
    await supabase.from('contact_messages').update({ behandeld: true }).eq('id', id);
    setMessages(prev => prev.map(m => m.id === id ? { ...m, behandeld: true } : m));
  }
  ...
  function senderName(m: Message): string {
    return m.fields.naam || m.fields.name || 'Onbekend';
  }

  function senderEmail(m: Message): string {
    return m.fields.email || '';
  }

  function previewText(m: Message): string {
    const bericht = m.fields.bericht || m.fields.message || m.fields.tekst || '';
    return bericht.length > 80 ? bericht.slice(0, 80) + '…' : bericht;
  }

  const filtered = messages.filter(m => {
    if (filter === 'open') return !m.behandeld;
    if (filter === 'behandeld') return m.behandeld;
    return true;
  });

  const openCount = messages.filter(m => !m.behandeld).length;
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="mb-6">
        <FormNotificationSubscribers formType="contact" />
      </div>

      {/* Header */}
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Contactberichten</h2>
          <p className="text-sm text-gray-400">
            {messages.length} totaal &middot; {openCount} open
          </p>
        </div>
        <div className="flex rounded-lg border border-gray-200 overflow-hidden text-sm">
          {(['all', 'open', 'behandeld'] as Filter[]).map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 transition-colors ${filter === f ? 'bg-gray-900 text-white' : 'bg-white text-gray-500 hover:bg-gray-50'}`}>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-dashed border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-400 text-sm">
            {filter === 'open' ? 'Geen openstaande berichten' : filter === 'behandeld' ? 'Nog geen behandelde berichten' : 'Nog geen berichten ontvangen'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((m) => {
            ...
            const naam = senderName(m);
            const email = senderEmail(m);
            const preview = previewText(m);
            const fieldEntries = Object.entries(m.fields).filter(([, v]) => v);

            return (
              <div key={m.id}
                className={`bg-white rounded-xl border overflow-hidden transition-shadow ${m.behandeld ? 'border-gray-100 opacity-75' : 'border-gray-200 hover:shadow-sm'}`}>

                {/* Header row */}
                <button className="w-full flex items-center gap-4 px-5 py-4 text-left"
                  onClick={() => setExpanded(open ? null : m.id)}>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${m.behandeld ? 'bg-gray-100' : 'bg-forest-100'}`}>
                    <span className={`font-bold text-sm ${m.behandeld ? 'text-gray-400' : 'text-forest-700'}`}>
                      {naam.charAt(0).toUpperCase()}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-900 text-sm truncate">{naam}</span>
                      {m.behandeld && (
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                      {preview && <span className="text-xs text-gray-400 truncate max-w-xs">{preview}</span>}
                      <span className="text-xs text-gray-300 shrink-0">{fmt(m.created_at)}</span>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-gray-100">
                    <div className="px-5 pt-4 pb-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {fieldEntries.map(([key, val]) => (
                        <div key={key} className={key === 'bericht' || key === 'message' || key === 'tekst' ? 'sm:col-span-2' : ''}>
                          <Detail
                            icon={key === 'email' ? <Mail className="w-3.5 h-3.5" /> : <MessageSquare className="w-3.5 h-3.5" />}
                            label={key.charAt(0).toUpperCase() + key.slice(1)}
                            value={val}
                          >
                            {key === 'email' ? (
                              <a href={`mailto:${val}`} className="text-forest-600 hover:underline text-sm">{val}</a>
                            ) : undefined}
                          </Detail>
                        </div>
                      ))}
                    ...
                    <div className="px-5 pb-4 flex flex-wrap gap-2">
                      {!m.behandeld && (
                        <>
                          <button onClick={() => setEmailModal(m)}
                          ...
                          <button onClick={() => markBehandeld(m.id)}
      ...
      {deleteConfirm && (
        <ConfirmModal onClose={() => setDeleteConfirm(null)}>
          <div className="text-center">
            ...
            <h3 className="font-semibold text-gray-900 mb-1">Bericht verwijderen?</h3>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => deleteMessage(deleteConfirm)}
...
function Detail({ icon, label, value, children }: { icon: React.ReactNode; label: string; value: string; children?: React.ReactNode }) {
  return (
    <div className="flex items-start gap-2.5 bg-gray-50 rounded-lg px-3 py-2.5">
      ...
      <div>
        ...
        {children ?? <div className="text-sm text-gray-800 whitespace-pre-wrap">{value}</div>}
      </div>
    </div>
  );
}
...
function MailBodyEditor({ initialHtml, onChange }: { initialHtml: string; onChange: (html: string) => void }) {
  ...
  return (
    <div className="flex flex-col border border-gray-200 rounded-xl overflow-hidden">
      <div className="flex items-center gap-0.5 px-3 py-2 bg-gray-50 border-b border-gray-200">
        ...
        <button type="button" title="Link invoegen"
      ...
      <div ref={ref} contentEditable suppressContentEditableWarning
        ...
        data-placeholder="Schrijf hier je reactie..."
        className={[
...
function EmailModal({ message, onClose, onSent }: { message: Message; onClose: () => void; onSent: (id: string) => void }) {
  const naam = message.fields.naam || message.fields.name || 'bezoeker';
  const email = message.fields.email || '';
  const onderwerp = message.fields.onderwerp || message.fields.subject || '';

  const defaultSubject = onderwerp ? `Re: ${onderwerp}` : 'Reactie op uw bericht';
  const defaultBody = `Hoi ${naam},<br><br>Bedankt voor je bericht! We hebben het in goede orde ontvangen.<br><br>`;
  ...
  async function send() {
    if (!email) { setError('Geen e-mailadres beschikbaar voor dit bericht.'); return; }
    setSending(true);
    setError('');
    try {
      ...
      // Mark as behandeld locally (edge function marks the membership_requests table, we do contact_messages here)
      await supabase.from('contact_messages').update({ behandeld: true }).eq('id', message.id);
      onSent(message.id);
    } catch (e: unknown) {
      ...
      <div className="relative bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl flex flex-col overflow-hidden"
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-gray-900 text-white shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold">Beantwoorden</span>
          </div>
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="flex items-center gap-2 bg-gray-100 rounded-full px-3 py-1">
                <div className="w-5 h-5 rounded-full bg-forest-200 flex items-center justify-center shrink-0">
                  <span className="text-[10px] font-bold text-forest-700">{naam.charAt(0).toUpperCase()}</span>
                </div>
                <span className="text-sm text-gray-800 font-medium">{naam}</span>
        ...
        <div className="shrink-0 px-5 py-3.5 border-t border-gray-100 bg-gray-50 flex items-center justify-between gap-3">
          {error ? (
            ...
            <span className="text-xs text-gray-400">
              Na verzending wordt het bericht automatisch op <strong>Behandeld</strong> gezet.
            </span>
          )}
          <button onClick={send} disabled={sending || !subject.trim() || !email}
